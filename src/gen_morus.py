...
ALGORITHMS = ["MORUS640", "MORUS1280"]


class MorusTestGenerator(aead_test_vector.AeadTestGenerator):

  def __init__(self, args):
    super().__init__(args.algorithm, args)
    if args.algorithm == "MORUS640":
      self.aead = morus.Morus640
    elif args.algorithm == "MORUS1280":
      self.aead = morus.Morus1280
    else:
  ...
  def generate_pseudorandom_vectors(self, keysizes: list[int],
    ...
    pseudorandom = flag.Flag(
        ...
        "Morus is a cipher that can benefit a lot from SIMD. "
        "Because of this we include test vectors for a large range of input "
        "sizes.")


    # cnt, keysize, ivsize, aadsize, msgsize
    # Tests all message sizes from 0 .. 161.
    # The range is quite large, since vectorizing MORUS using AVX-512 may
    # include some tricky unrolling. This may lead to errors that only
    # occur after several plaintext (or AAD) blocks.
    self.generate_pseudorandom(
        1, keysizes, ivsizes, [0], list(range(162)), flags=[pseudorandom])
    # Tests all aad sizes from 0 .. 161.
    self.generate_pseudorandom(
        1, keysizes, ivsizes, list(range(162)), [16], flags=[pseudorandom])
    # longer message size
    self.generate_pseudorandom(
        1,
        keysizes,
        ivsizes, [0], [223, 224, 225, 255, 256, 257, 511, 512, 513],
        flags=[pseudorandom])
    # longer aad size
    self.generate_pseudorandom(
        1,
        keysizes,
        ivsizes, [223, 224, 225, 255, 256, 257, 511, 512, 513], [20],
        flags=[pseudorandom])
    self.generate_pseudorandom(
  ...
  def generate_all(self, args):
    ...
    keysizes = self.aead.key_sizes_in_bytes
    ivsizes = [16]
...
class MorusProducer(producer.Producer):
  ...
  def generate_test_vectors(self, namespace):
    tv = MorusTestGenerator(namespace)
...
def main(namespace):
  MorusProducer().produce(namespace)


if __name__ == "__main__":
  MorusProducer().produce_with_args()

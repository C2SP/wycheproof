...
# ====== Variables =====
# Plan:
# state00 = Var('state[0][0]')
# state01 = Var('state[0][1]')
# state11 = Var('state[1][1]', AesRound(state00, state01))
# state11.set_value(bytes(range(16)))
# state01.set_value(bytes(16))
# state00.val
#

class Expr:
  def __init__(self, value=None, refs=[]):
    self.val = value
    self.refs = refs
  def __bool__(self):
    return self.val is not None
  def __repr__(self):
    return f"Expr({repr(self.name)}, {repr(self.value)})"
  def __str__(self):
    return f"{self.name}: {self.value})"
  def set_value(self, val):
    if self.val is not None:
      raise ValueError("can't set variable twice")
    self.val = val
    for v in self.refs:
      v.try_solve(self)
...
class AesRound(Expr):
  def __init__(self, x, y):
    ...
    self.y = y
  def try_solve(self):
    if self.x and self.y and not self:
      self.set_val(aes_util.aes_enc(self.x.val, self.y.val))
    elif self.x and self and not self.y:
      self.y.set_val(aes_util.aes_enc(self.x.val, self.val)
    elif self.y and self and not self.x:
      self.x.set_val(aes_util.aes_inv_enc(self.val, self.y.val)
  def __repr__(self):
    return f"AesRound({repr(self.x)}, {repr(self.y)})"
...
class Xor(Expr):
  ...
  def try_solve(self):
    if self.x and self.y and not self:
      self.set_val(xor(self.x.val, self.y.val))
    elif self.x and self and not self.y:
      self.set_val(xor(self.x.val, self.val))
    elif self.y and self and not self.x:
      self.set_val(xor(self.y.val, self.val))
  def __repr__(self):
    return f"Xor({repr(self.x)}, {repr(self.y)})"

...
class RsaOaepTest(googletest.TestCase):

  def testRsaOaep(self):
    ...
    expected_schema = "rsaes_oaep_decrypt_schema.json"
    cnt = 0
    total_errors = 0
    total_files = 0
    skipped_keys = 0
    for fname, test in test_util.get_all_test_vectors("^rsa_oaep.*test\.json$",
                                                      expected_schema):
      errors = 0
      verified = 0
      total_files += 1
      ...
      for g in test["testGroups"]:
        ...
        for t in g["tests"]:
          ...
          try:
            ...
            if "InvalidOaepPadding" in t["flags"]:
